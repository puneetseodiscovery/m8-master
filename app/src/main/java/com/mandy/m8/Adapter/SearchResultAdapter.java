...
public class SearchResultAdapter extends RecyclerView.Adapter<SearchResultAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder viewHolder, final int i) {
        ...
        if (CollectionUtils.isEmpty(datum.getImages())) {
            ...
            Glide.with(context).load("http://m8.amrdev.site/public/upload/items/" + datum.getImages().get(0).getImage()).listener(new RequestListener<Drawable>() {
                @Override
...
//            Glide.with(context).load("http://m8.amrdev.site/public/upload/items/" + datum.getImages().get(0).getImage()).dontAnimate().dontTransform().into(viewHolder.imageView);
        }
    }
